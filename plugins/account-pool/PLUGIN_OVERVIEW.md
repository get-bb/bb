...
The hub runs inside BB and serves an Anthropic Messages endpoint and an OpenAI Responses endpoint. With routing on, BB hands the Claude Code or Codex process a base URL that points at the hub and a token scoped to that machine, and the provider reports **Proxied** in its health row. An account is skipped for a request when it is at or above the switch threshold or in error. The threshold defaults to 98 percent of a window. A refusal first rechecks exhausted accounts, so upgrades apply next turn. Account secrets stay in the BB data directory on the server machine, and the hub refreshes them in the background. An account enters error only when its OAuth refresh token or API key is rejected. A 401 or 403 on a freshly refreshed OAuth token is treated as an upstream outage: the request gets HTTP 503 and the token sits out of routing for one minute. `bb pool account refresh <id>` retries an errored OAuth account and clears the error when a new token is issued.
