...
const contract = defineRpcContract({
  "provider-usage.v1.get": {
    experimental_description:
      "Returns local Codex account usage for each host. refresh=true requests fresh collection. Host failures are returned individually; observedAt remains the time of the last successful measurement.",
...
export function registerUsageSource(bb: BbPluginApi) {
  ...
  bb.rpc.register(
    ...
    },
    {
      ...
      experimental_description:
        "Codex usage from host-local credentials. Independent of pooled accounts and display plugins.",
    },
  );
}
