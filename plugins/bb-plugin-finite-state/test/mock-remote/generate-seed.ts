...
function buildCorpus(seed: string): {
  ...
  const taraComponents = Array.from({ length: COUNTS.taraNodes }, (_, index) => ({
    ...
    projectId,
    kind: "component",
    reviewVersion: (100 + index).toString(),
  ...
  const firmwarePaths = Array.from({ length: COUNTS.firmwarePaths }, (_, index) => {
    ...
    return {
      path,
      kind,
      hash: kind === "symlink" ? null : seededHex(seed, `firmware-file-${number}`),
      size: index === 0 ? 0 : kind === "symlink" ? null : 16 + (index % 8),
      linkTarget: kind === "symlink" ? "../../outside-root" : null,
      byteSample: index === 0 ? "firmware/bytes/zero-byte.bin" : index === 1 ? "firmware/bytes/eagled.bin" : null,
  ...
  const forgeJobs = ["RUNNING", "COMPLETED", "FAILED", "TIMEOUT"].map((status, index) => ({
    ...
    error: status === "FAILED" || status === "TIMEOUT" ? { code: `FORGE_JOB_${status}`, message: `Fixture ${status.toLowerCase()}` } : null,
  }));
  ...
  const cases: FixtureManifest["cases"] = {
    ...
    "non-ascii-names": {
      description: "Names contain composed non-ASCII Latin and Greek characters.",
  ...
  const drafts: FileDraft[] = [
    ...
    { path: "platform/findings.jsonl", bytes: jsonl([...findings, duplicateFinding]), rows: findings.length + 1 },
    ...
    { path: "firmware/bytes/zero-byte.bin", bytes: new Uint8Array() },
...
async function createGeneratedDirectory(seed: string, root: string): Promise<FixtureManifest> {
  const { drafts, cases } = buildCorpus(seed);
...
export async function generateFixtureCorpus(options: GenerateOptions): Promise<FixtureManifest> {
  ...
  const stagingRoot = await mkdtemp(join(parent, `.${basename(outDir)}.generate-`));
...
function parseCli(argv: string[]): GenerateOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else throw new FixtureGenerationError("INVALID_OUTPUT", `Unknown or incomplete argument: ${argument}`);
  }
...
if (invokedPath === import.meta.url) {
  generateFixtureCorpus(parseCli(process.argv.slice(2))).catch((error: unknown) => {
    const message = error instanceof Error ? `${error.name}: ${error.message}` : "Fixture generation failed";
