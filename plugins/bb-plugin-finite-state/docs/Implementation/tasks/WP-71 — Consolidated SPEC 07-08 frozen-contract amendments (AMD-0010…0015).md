...
5. **AMD-0014 — dependencies.** Declare `kicadts` and `@google/model-viewer` as the reviewed batch. `@xyflow/react` is already declared. `kicad-cli`, Python tooling, and instrument SDKs are host prerequisites behind `needsConfiguration`, not npm dependencies.
