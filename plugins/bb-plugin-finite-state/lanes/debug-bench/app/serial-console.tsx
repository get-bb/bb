...
function LoadingConsole(): React.JSX.Element {
  return (
    <div className="space-y-3 p-3" data-state="loading" role="status" aria-label="Loading serial console">
...
function EmptyConsole(): React.JSX.Element {
  return (
    <div className="flex min-h-72 items-center justify-center p-5 text-center" data-state="empty">
      <div>
        <Icon className="mx-auto size-5 text-muted-foreground" name="Terminal" />
        <p className="mt-2 text-sm font-medium">No serial port available</p>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          Connect a UART adapter, install pyserial if prompted, then rescan the bench.
        </p>
      </div>
    </div>
  );
...
export function SerialConsole({
  ...
  const [deviceId, setDeviceId] = useState<string | null>(serialDevices[0]?.deviceId ?? null);
  ...
  const loadSession = useCallback(async () => {
    ...
    try {
      const current = await rpc.call("benchDevSerialSessionCurrent", { ...scope, deviceId });
  ...
  }, [deviceId, filter, loadSession, paused, projectId, projectVersionId, serialDevices.length]);

  useRealtime("serial:changed", (hint) => {
    if (
      hint !== null && typeof hint === "object" &&
      Reflect.get(hint, "deviceId") === deviceId && !pausedRef.current
    ) void loadSession();
  });
      ...
      <div className="p-3" data-state="error">
        <Alert className="border-destructive/40">
          ...
          <AlertDescription className="space-y-3">
            <p>{state.message}</p>
            <Button onClick={() => { setState({ kind: "loading" }); void loadSession(); }} size="sm" variant="outline">
              Retry console
            </Button>
          </AlertDescription>
        </Alert>
  ...
  return (
    <section className="flex h-[34rem] min-h-0 flex-col" data-state={state.connection}>
      <header className="space-y-2 border-b border-border bg-muted/20 p-2.5">
        ...
        <div className="flex gap-2">
          <Input
            ...
            value={filter}
          />
          <Button onClick={() => setPaused((value) => {
          ...
          {state.connection === "connected" || state.connection === "reconnecting" ? (
            <Button disabled={busy} onClick={() => deviceId && void perform(async () => {
              await rpc.call("benchDevSerialSessionClose", { ...scope, deviceId });
            })} size="sm" variant="outline">
              Close
            </Button>
          ) : (
            <Button disabled={busy} onClick={() => deviceId && void perform(async () => {
              await rpc.call("benchDevSerialSessionOpen", { ...scope, deviceId, baud: 115_200 });
            })} size="sm">
              Connect
            </Button>
          )}
      ...
      {state.connection === "unconfigured" ? (
        <Alert className="m-2 border-border" data-state="unconfigured">
          ...
          <AlertDescription>
            {state.message ?? "Python with pyserial is required. Review the serial helper in the device registry."}
          </AlertDescription>
        </Alert>
      ) : null}
      {state.connection === "reconnecting" ? (
        <div className="border-b border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground" role="status">
          Reconnecting with capped backoff. Capture continues when the port returns.
        </div>
      ) : null}
      {state.message && state.connection === "closed" ? (
        <div className="border-b border-border px-3 py-2 text-xs text-muted-foreground">{state.message}</div>
      ) : null}
      {actionError ? (
        <div className="border-b border-destructive/30 px-3 py-2 text-xs text-destructive" role="alert">{actionError}</div>
      ) : null}
      {gaps.map((gap, index) => (
        <div className="border-b border-border bg-muted/20 px-3 py-1.5 text-xs text-muted-foreground" key={`${gap.afterCursor}-${index}`}>
          <Icon className="mr-1 inline size-3.5" name="AlertCircle" />
          {gap.dropped.toLocaleString()} lines dropped before cursor {gap.afterCursor.toLocaleString()}
        </div>
      ))}
      <div className="min-h-0 flex-1 overflow-auto bg-background font-mono text-xs" ref={viewport}>
        <div className="relative w-full" style={{ height: virtualizer.getTotalSize() }}>
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            return (
              ...
              >
                <span className="tabular-nums text-muted-foreground">{line.cursor}</span>
                <span className={line.dir === "tx" ? "text-primary" : "text-muted-foreground"}>{line.dir}</span>
