...
describe("ArtifactList", () => {
  ...
  it("offers recovery instead of a download when hash verification is absent", () => {
    const view = render(<ArtifactList runId="run-1" artifacts={[{ name: "evidence.bin", kind: "binary", sha256: null, bytes: null }]} />);
  ...
  it("offers recovery when the logical-locator byte adapter is unavailable", () => {
    const view = render(<ArtifactList runId="run-1" artifacts={[{ name: "evidence.bin", kind: "binary", sha256: "a".repeat(64), bytes: 42, downloadAvailable: false }]} />);
    expect(view.getByText("Recovery needed")).toBeTruthy();
    expect(view.queryByRole("link")).toBeNull();
  });
});
