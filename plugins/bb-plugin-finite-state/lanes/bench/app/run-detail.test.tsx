...
function detail(state: "fresh" | "stale" = "fresh") {
  return {
    projectId: "p1", projectVersionId: "v1", kind: "verificationRun", key: "run-1", label: "tier0 completed",
    fields: {
      tier: "tier0", status: "completed", target: "REQ-1", firmwareDigest: "a".repeat(64), threadId: "thread-1", config: { mode: "static" },
      results: [{ requirementId: "REQ-1", checkId: "CHECK-1", outcome: "pass", evidenceSummary: "Verified" }],
      artifacts: [{ name: "report.json", kind: "evidence", sha256: "b".repeat(64), bytes: 12 }],
      attestations: [{ format: "in-toto", subjectDigest: "a".repeat(64), verified: true }],
    }, links: [], cache: { ...cache, state, message: state === "stale" ? "cached" : null },
  };
}
