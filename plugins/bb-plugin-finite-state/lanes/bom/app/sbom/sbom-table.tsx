...
type BomSoftwarePage = z.output<(typeof rpcContract)["bomSoftwareList"]["output"]>;
...
function realtimeProjectVersion(value: unknown): string | null {
  return typeof value === "object" && value !== null &&
    "projectVersionId" in value && typeof value.projectVersionId === "string"
...
function reachabilityValue(value: JsonValue | undefined): SbomRowView["reachability"] {
...
function toRow(item: BomSoftwareItem): SbomRowView {
  ...
  return {
    ...
    findings: findingsValue(fields.findings),
  };
}
...
export function SbomTable({
  ...
  const serializedFilters = useMemo(() => JSON.stringify(rpcFilters(filters)), [filters]);
  ...
  const refresh = useCallback(async (keepVisible = false) => {
    ...
    try {
      ...
      if (generation !== requestGeneration.current) return;
      setError(cause instanceof Error ? cause.message : "The SBOM page could not be loaded.");
    } finally {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (generation === requestGeneration.current) {
        setPageError(cause instanceof Error ? cause.message : "The next page could not be loaded.");
      }
    } finally {
  ...
  const openFinding = useCallback((stableKey: string) => {
    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="max-w-md rounded-lg border border-destructive/40 bg-card p-5 text-center">
          <Icon aria-hidden="true" className="mx-auto size-6 text-destructive" name="AlertCircle" />
          <h3 className="mt-3 font-semibold">Software inventory unavailable</h3>
          <p className="mt-1 text-sm text-muted-foreground">{error}</p>
          <Button className="mt-4" onClick={() => void refresh()} variant="outline">Retry</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="max-w-md rounded-lg border border-border bg-card p-5 text-center">
          <Icon aria-hidden="true" className="mx-auto size-6 text-muted-foreground" name="PackageReceive" />
          ...
          <Button className="mt-4" onClick={() => void refresh()} variant="outline">Retry query</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {cache?.state === "stale" || error ? (
        <div className="flex items-center gap-2 border-b border-destructive/40 bg-muted px-3 py-2 text-sm" role="status">
          ...
          <Button onClick={() => void refresh(true)} size="sm" variant="outline">Retry</Button>
        </div>
      ) : null}
      <div className="grid h-9 shrink-0 grid-cols-12 items-center gap-2 border-b border-border bg-muted/60 px-2 text-xs font-medium text-muted-foreground" role="row">
        <span className="col-span-3 pl-7" role="columnheader">Component</span>
        <span className="col-span-1" role="columnheader">Version</span>
        <span className="col-span-1" role="columnheader">License</span>
        <span className="col-span-3" role="columnheader">Severity</span>
        <span className="col-span-1 text-right" role="columnheader">KEV</span>
        <span className="col-span-1" role="columnheader">Reachability</span>
        <span className="col-span-1 text-right" role="columnheader">Files</span>
        <span className="col-span-1 text-right" role="columnheader">Links</span>
      </div>
      <div
      ...
      <div className="flex h-9 shrink-0 items-center gap-3 border-t border-border px-3 text-xs text-muted-foreground">
        <span>{rows.length.toLocaleString()} loaded{total === null ? "" : ` of ${total.toLocaleString()}`}</span>
        {loadingMore ? <span>Loading next page…</span> : null}
        {next && !pageError && !loadingMore ? (
          <Button className="ml-auto" onClick={() => void loadMore()} size="sm" variant="ghost">
            Load next page
          </Button>
        ) : null}
        {pageError ? (
          <>
            <span className="min-w-0 flex-1 truncate text-destructive">Next page failed: {pageError}</span>
            <Button onClick={() => {
              setPageError(null);
              void loadMore();
            }} size="sm" variant="outline">Retry page</Button>
          </>
        ) : null}
      </div>
    </div>
