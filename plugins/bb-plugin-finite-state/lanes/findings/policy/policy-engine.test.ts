...
function insertFinding(db: Database.Database, id: string, options: FindingOptions = {}): string {
  ...
  const component = { purl: `pkg:generic/${name}@1.0.0`, name, group: null, version: "1.0.0" };
...
`)).toThrow(/overwrite_existing/u);
  ...
  it("holds invalid NOT_AFFECTED proposals instead of writing them", () => {
    ...
    const invalid = parseTriagePolicy({
      schema: "fs-triage-policy/v1",
      rules: [{ name: "invalid", when: {}, set: { status: "NOT_AFFECTED", reason: "No rationale" } }],
      holdback: [],
      options: { overwrite_existing: false },
    });
  ...
  it("writes through WP-27 and is idempotent when the overlay index has not rebuilt yet", async () => {
    ...
    insertFinding(db, "1");
    const directory = await root();
    const first = await applyPolicy({ db, root: directory, policy: policy() }, scope, { dryRun: false });
    ...
    const second = await applyPolicy({ db, root: directory, policy: policy() }, scope, { dryRun: false });
  ...
  it("applies the exact preview candidate set and honors the policy digest CAS", async () => {
    ...
    const directory = await root();
    const writer = vi.fn(async (_root: string, input: Parameters<NonNullable<Parameters<typeof applyPolicy>[0]["setDecision"]>>[1]) => fakeWrite(input.stableKey));
    const digest = createHash("sha256").update(JSON.stringify(activePolicy)).digest("hex");

    await expect(applyPolicy(
      { db, root: directory, policy: activePolicy, expectedPolicySha256: "f".repeat(64), evaluated: preview, setDecision: writer },
      scope,
      { dryRun: false },
    ...
    const report = await applyPolicy(
      { db, root: directory, policy: activePolicy, expectedPolicySha256: digest, evaluated: preview, setDecision: writer },
      scope,
      { dryRun: false },
    );
  ...
  it("keeps 39 successes when one item has a retryable CAS conflict", async () => {
    ...
    const report = await applyPolicy({ db, root: directory, policy: policy(), setDecision: writer }, scope, { dryRun: false });
  ...
  it("retries OVERLAY_LOCK_HELD with bounded backoff and does not retry CAS", async () => {
    ...
    expect(report).toMatchObject({ written: 1, errors: [] });
    expect(sleep.mock.calls.map(([milliseconds]) => milliseconds)).toEqual([10, 20]);
