...
import { parseVendorVex, type VendorVexFormat } from "./parse.js";
...
function resolutionFor(deps: ImportDeps, statement: NormalizedVendorStatement): FindingResolution {
  return resolveFinding(deps.db, {
    schema: "fs-finding-key/v1",
    project: deps.projectId,
    cve: statement.cve,
    ...statement.component,
  }, deps.pvId, "any_version");
}

function findingIds(resolution: FindingResolution): Set<string> {
  return new Set(resolution.state === "resolved" ? resolution.rows.map((row) => row.findingId) : []);
}
...
function localDecisionExists(
  ...
  return rows.some((row) => {
    ...
    const localResolution = resolveFinding(deps.db, {
      schema: "fs-finding-key/v1",
      project: decoded.project,
      cve: decoded.identity.cve,
      ...component,
    }, deps.pvId, pin);
    return localResolution.state === "resolved"
...
function prepare(
  ...
  const targetStableKey = matched ? resolution.rows[0]?.stableKey ?? null : null;
  const needsCompletion = statement.status === "NOT_AFFECTED" && statement.justification === null;
  const keptLocal = localDecisionExists(deps, statement, resolution, authoredComponents) && !options.overwrite;
  ...
  const proposal: VendorProposalV1 = {
    ...
    source: {
      ...
      statement: statement.sourceRef.slice(statement.sourceRef.indexOf("#") + 1),
    },
  };
  return {
    input: {
    ...
    stableKey: targetStableKey ?? stableKeyFor(deps.projectId, statement.component, statement.cve),
...
export async function importVendorVex(
  ...
  if (options.vendor.trim().length === 0 || options.vendor.length > 500) {
    throw new TypeError("Vendor name must contain between 1 and 500 characters");
  }
  const parsed = await parseVendorVex(file);
  const mapped = mapVendorVex(parsed);
  const overlays = await readOverlayFiles(deps.root);
  if (overlays.errors.length > 0) {
    throw new Error(`Vendor import cannot safely inspect local decisions: ${overlays.errors[0]?.message ?? "overlay parse failed"}`);
  }
  const authoredComponents = new Map<string, VendorProposalInput["component"]>();
  for (const overlay of overlays.files) {
    ...
    for (const cve of Object.keys(overlay.overlay.decisions)) {
      authoredComponents.set(stableKeyFor(deps.projectId, overlay.overlay.component, cve), overlay.overlay.component);
    }
  }
  ...
  for (const statement of mapped.statements) {
    try {
      prepared.push(prepare(deps, parsed, statement, options, authoredComponents));
    } catch (error) {
      errors.push({
        sourceRef: statement.sourceRef,
        code: "MAPPING_FAILED",
  ...
  for (const item of prepared) {
    if (item.keptLocal) {
      proposals.push({ stableKey: item.stableKey, state: "kept_local", sourceRef: item.sourceRef });
      continue;
    }
    if (options.dryRun) {
      proposals.push({
    ...
    try {
      ...
      proposals.push({ stableKey: item.stableKey, state: item.input.proposal.state, sourceRef: item.sourceRef });
    } catch (error) {
      errors.push({
        sourceRef: item.sourceRef,
  ...
  if (!options.dryRun && written > 0) await rebuildOverlayIndex(deps.db, deps.root);

  return {
    source: { format: parsed.format, digest: parsed.digest, vendor: options.vendor },
