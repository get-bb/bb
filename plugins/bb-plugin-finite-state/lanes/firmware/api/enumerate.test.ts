...
async function fixture(fetch: typeof globalThis.fetch, limiter?: RemoteLimiter) {
...
describe("API firmware enumeration", () => {
  it("recurses through the limiter with depth one calls and clamps the crawl at eight levels", async () => {
    ...
    try {
      ...
      expect(calls.every((call) => call.depth === "1")).toBe(true);
