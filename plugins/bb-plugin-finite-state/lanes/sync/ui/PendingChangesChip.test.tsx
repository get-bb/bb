...
describe("PendingChangesChip", () => {
  it("leaves safe route segments raw for host navigation encoding", async () => {
    ...
    expect(
      syncScopeSubPath({ projectId: "qa-project", pvId: null }),
    ).toBe("scope/qa-project/@project");
    expect(() =>
  ...
  it("shows local and conflict counts and opens the validated scoped route", async () => {
    const slot = renderSlot(await chipRegistration(), {}, {
      rpc: {
        syncStatus: () => ({
    ...
    expect(slot.inspection.navigateCalls).toEqual([
      {
        ...
        options: {
          subPath:
            "scope/platform-project/version-7/surface/vexDecision",
        },
      },
    ]);
