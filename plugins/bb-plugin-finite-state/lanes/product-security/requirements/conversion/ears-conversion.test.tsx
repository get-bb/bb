...
function source(id: string, description = `Original prose for ${id}`): ConversionSource {
  return {
    requirementId: id,
    remoteId: `remote-${id}`,
...
function snapshot(sources: ConversionSource[]): ConversionPullSnapshot {
  return {
    ...
    references: {
      requirements: new Map(sources.map((item) => [item.requirementId, item.remoteId])),
      checks: new Map(sources.flatMap((item) => item.checks.map((check) => [check.slug, check.id]))),
...
function requirement(item: ConversionSource, pattern: EarsPattern = "ubiquitous"): RequirementYamlV1 {
...
function fixture(initial: ConversionPullSnapshot) {
  ...
  return {
    deps,
    files,
    setSnapshot(next: ConversionPullSnapshot) { current = next; },
  };
}
...
describe("EARS conversion flow", () => {
  it("bundle pages grounded source with bounded evidence summaries", async () => {
    const sources = Array.from({ length: 21 }, (_, index) => source(`REQ-${index + 1}`));
    const test = fixture(snapshot(sources));
    const meta = await buildConversionBundle(test.deps);
    expect(meta).toMatchObject({ projectId: "project-1", pulledAt: PULLED_AT, requirementIds: sources.map((item) => item.requirementId) });
    ...
    await expect(getConversionBundlePage(meta.bundleId, first.nextCursor ?? undefined)).resolves.toMatchObject({ nextCursor: null, items: [{ requirementId: "REQ-21" }] });
  });

  it("spawn context is bounded and covers six patterns and prohibitions", async () => {
    const test = fixture(snapshot([source("REQ-1", "SECRET SOURCE BODY MUST NOT BE IN PROMPT")]));
    const meta = await buildConversionBundle(test.deps);
    const prompt = buildConversionPrompt(meta, ["product-security/requirements/REQ-1.yaml"]);
  ...
  it("three valid requirements pass gates 1–2 and await human", async () => {
    ...
    const patterns: EarsPattern[] = ["ubiquitous", "event_driven", "state_driven"];
    sources.forEach((item, index) => test.files.set(item.targetPath, serializeRequirement(requirement(item, patterns[index] ?? "ubiquitous"))));
    const gates = await validateConversion(sources.map((item) => item.targetPath));
    expect(gates).toHaveLength(3);
    expect(gates.every((gate) => gate.schema.ok && gate.roundTrip.ok)).toBe(true);
  ...
  it("wires only the three conversion RPCs and fails closed on requested review", async () => {
    ...
    ).run("project-1", "version-1", "generation-1", PULLED_AT, PULLED_AT, PULLED_AT);
    db.prepare(
    ...
    db.prepare(
      ...
      "project-1", "version-1", "generation-1", requirementKey, item.remoteId,
      JSON.stringify({ reqId: item.requirementId, description: item.sourceDescription, priority: "P1" }),
      "b".repeat(64), PULLED_AT,
    );
    db.prepare(
      `INSERT INTO id_map
         (project_id, project_version_id, entity_kind, generation_id, entity_key, remote_id, pulled_at)
       VALUES (?, ?, 'requirement', ?, ?, ?, ?)`,
    ).run("project-1", "version-1", "generation-1", requirementKey, item.remoteId, PULLED_AT);
    db.prepare(
      ...
      "project-1", "version-1", "generation-1", item.checks[0]?.id, item.checks[0]?.slug,
      item.checks[0]?.description, item.checks[0]?.passCriteria, item.checks[0]?.failCriteria,
      JSON.stringify({ tier: "static" }), PULLED_AT,
    );
    db.prepare(
    ...
    const started = rpcContract.earsConversionStart.output.parse(await host.harness.callRpc("earsConversionStart", {
  ...
  it("agent invention and derived fields fail gates with file and line", async () => {
    ...
    if (!contract) throw new Error("The fixture must contain a verification contract.");
    ...
    expect(gate?.schema.errors).toEqual(expect.arrayContaining([expect.objectContaining({ code: "DERIVED_FIELD", artifactId: item.targetPath, line: expect.any(Number) })]));
    expect(gate?.roundTrip).toMatchObject({ ok: false, unresolved: ["CHECK-INVENTED"] });
  });
  ...
  it("drift rerun scopes changed inputs and staleness requires rebuild", async () => {
    ...
    test.files.set(second.targetPath, serializeRequirement(requirement(second)));
    test.setSnapshot(snapshot([first, source("REQ-2", "Upstream changed") ]));
    await expect(detectConversionDrift(meta.bundleId)).resolves.toMatchObject({ driftedRequirementIds: ["REQ-2"], unchangedRequirementIds: ["REQ-1"] });
    await expect(buildDriftRerunBundle(meta.bundleId)).resolves.toMatchObject({ requirementIds: ["REQ-2"] });
    const gates = await validateConversion([first.targetPath, second.targetPath]);
    expect(gates.find((gate) => gate.requirementId === "REQ-1")?.roundTrip.staleSource).toBe(false);
