...
function parseModel(fields: UnknownRecord): RunDetailModel {
  ...
  return {
    projectVersionId: text(fields.projectVersionId), requirementId: text(fields.requirementId) ?? "unknown requirement", tier: text(fields.tier) ?? "unknown",
...
export function VerificationRunDetailLayer({ projectId, detail }: Props): React.JSX.Element | null {
  ...
  return <RunDetail error={error} jobState={jobState} loadingHistory={loadingHistory} model={model} onLoadMore={() => {
    ...
    void rpc.call("verificationResultHistoryList", { projectId, projectVersionId: model.projectVersionId, requirementId: target.requirementId, tier: target.tier, pageSize: 50, continuation: model.historyNext }).then((page) => setModel((current) => current ? { ...current, history: [...current.history, ...page.items.map((item) => historyItem(item.fields))], historyTotal: page.total, historyNext: page.next } : current)).catch((cause) => setError(cause instanceof Error ? cause.message : "History page failed.")).finally(() => setLoadingHistory(false));
  }} onRetry={() => setRevision((value) => value + 1)} onRun={(checkId) => {
    setRunning(true); setJobState("QUEUED");
    void rpc.call("verificationsRunStart", { projectId, projectVersionId: model.projectVersionId, requirementId: target.requirementId, tier: target.tier, checkId, parameters: {} }).then((job) => { setJobState(job.state); setRevision((value) => value + 1); }).catch((cause) => setError(cause instanceof Error ? cause.message : "Verification invocation failed.")).finally(() => setRunning(false));
