...
- Prior artifact hashes:
  - `shared/contract.ts`: `84bee6cab373316b2c4e47707c1c80b7a54a007d9ae2bf46862faad9cba8e905`
  - `lib/remote/config.ts`: `eaf861403b7eb432c3312fc6aadaead7ca636d769398224073e4286698527a08`
...
- Issuance options:
  1. Add a narrow issuer RPC to `shared/contract.ts`. By itself this can issue an id only for an already authenticated host file identity; it satisfies opaque-id issuance and the frozen action boundary, but it does not satisfy safe selection outside the worktree unless paired with a host-owned picker/identity capability. It must never accept a raw absolute browser path.
  2. Add a host-safe file-selection SDK surface that returns an opaque, host-bound, short-lived file capability to the plugin backend, then add the narrow issuer RPC that exchanges that capability for `inputId`. This satisfies the materialize dialog's host-safe selection requirement, external-file support, opaque-id boundary, cancellation, and local-unpack default. For non-server execution hosts, the host contract must also keep unpack execution co-located with the selected file; returning an absolute path to the server is not sufficient.
  3. Add a CLI issuance path for `bb finite-state firmware pull <pv-id> --image <firmware-file>`. This can satisfy CLI-initiated local unpack when the CLI and unpack runtime are explicitly co-located and the invoking host resolves the file, but it cannot satisfy the materialize dialog, native safe-selection workflow, or local-image-as-default UI acceptance criteria. It is therefore a useful agent surface, not a complete WP-51 solution.
- Recommendation: approve option 2 with the smallest host-owned capability contract plus a narrow plugin issuer RPC. Keep option 3 as the required CLI peer, but do not treat it as a substitute for the UI flow. The capability must bind host, project, project version, environment/worktree, selected regular file, expiry, and one-time consumption without exposing an absolute path to frontend-visible data or agent messages.
- Required settings additions: add `standaloneUnpackExecutablePath: { type: "string", label: "Standalone unpack wrapper", default: "" }` and `standaloneUnpackImage: { type: "string", label: "FACT extractor image", default: "localhost:5000/services-unpack:latest" }` to `REMOTE_SETTING_DESCRIPTORS`; add the corresponding required fields to `RemoteSettingValues`; parse the wrapper to `string | null` and the image to a non-empty explicit string in `RemoteConfig`; and expose change detection so the production firmware registration reconfigures the existing runtime without defining a second settings owner. The implementation must update the CLI guide and generated plugin-command skill surfaces for these user-facing knobs.
- Migration: after owner approval, add the selected host-safe capability and issuer boundary, increment the RPC contract version, update frozen baselines, and wire the existing registry/runtime from `lanes/firmware/register.ts`. Extend the sole remote settings owner with the two descriptors and propagate explicit values to firmware registration. Add focused authorization, expiry, host/scope mismatch, one-time-use, reconnect, settings-change, and path-redaction tests. Preserve the existing `firmwareMaterializeStart` opaque-id action and never add raw browser paths to it.
...
- Blocked pending approval: host-safe local-image selection and token issuance, production `configureStandaloneUnpackRuntime(...)`, and a truthful end-to-end Local image submit from the materialize dialog. These blocked pieces prevent WP-51 from meeting its default-local-action and extractor-prerequisite acceptance criteria, but do not block the API fallback or read-only firmware UX.
- Affected WPs and gates: WP-48, WP-51, WP-61, WP-64, WP-67; firmware materialization UI/CLI, remote settings, shared RPC and host capability contracts, frozen guards; G6
- Contract owner: pending
- Affected-lane reviewer: pending
- Broadcast and merge commits: pending
- Evidence: FS-65 scope audit at base `afb16ac928e053187b5bfe85ace2e8b7887ed751`; `shared/contract.ts` exposes `firmwareMaterializeStart.inputId` but no issuer; `lanes/firmware/register.ts` contains the unconfigured registry/runtime seams; `PluginAppBuilder` exposes no file picker and the backend SDK exposes only `hosts.pickFolder`; `lib/remote/config.ts` is the frozen sole settings descriptor owner and has no standalone unpack fields; no standalone wrapper exists in this repository.
