...
- Considered issuance options:
  1. A plugin-owned issuer RPC is the approved primary path. Add `firmwareInputIssue` with strict input `{ projectId: identifier, projectVersionId: projectVersionId, environmentId: identifier, firmwarePath: relativeArtifact }` and output `{ projectId, projectVersionId, inputId: identifier, fileName: string, expiresAt: timestamp }`. The handler must resolve the named environment through `bb.sdk`, verify its project, resolve `firmwarePath` beneath that environment's worktree without traversing symlinks, require a readable regular file, and call `getStandaloneUnpackInputRegistry().issue(...)`. Neither input nor output permits an absolute host path. The dialog can therefore issue its own opaque token for a confined workspace file identity and submit the existing `firmwareMaterializeStart` action. An image outside the worktree remains unsupported until a separate, owner-approved host contract exists; the UI must say so rather than accepting a raw path.
...
- Authorization: classify `firmwareInputIssue` as `action`. Issuance grants a scoped, expiring, one-time local capability and therefore is not a read, but it does not itself materialize bytes or mutate the firmware cache; `firmwareMaterializeStart` remains the state-changing action. Classifying issuance as `human-only` would make the primary UI unreachable because the frozen human-only capability mint is intentionally unavailable. Local RPC origin checks, environment/project/path confinement, regular-file verification, expiry, scope binding, and one-time consumption are the authorization controls. Add the method to the frozen wire-name and authorization maps and increment `CONTRACT_VERSION` from 1 to 2 in the approved implementation.
...
- Migration: after independent affected-lane review, add `firmwareInputIssue` exactly as specified, increment `CONTRACT_VERSION` to 2, update frozen baselines through the amendment accept flow, and wire the existing registry/runtime from `lanes/firmware/register.ts`. Extend the sole remote settings owner with the two descriptors and propagate explicit values to firmware registration. Add focused authorization, expiry, environment/project/path mismatch, symlink/regular-file, one-time-use, settings-change, and path-redaction tests. Preserve the existing `firmwareMaterializeStart` opaque-id action and never add raw browser paths to it.
...
- Evidence: FS-65 scope audit at base `afb16ac928e053187b5bfe85ace2e8b7887ed751`; `shared/contract.ts` exposes `firmwareMaterializeStart.inputId` but no issuer; `lanes/firmware/register.ts` contains the unconfigured registry/runtime seams; `PluginAppBuilder` exposes no file picker and the backend SDK exposes only `hosts.pickFolder`; `lib/remote/config.ts` is the frozen sole settings descriptor owner and has no standalone unpack fields; no standalone wrapper exists in this repository; owner scope decision recorded on FS-65 on 2026-08-13.
