...
- `bb.branding.experimental_icons` (optional) — the plugin's own icon
  ...
  `bb.experimental_machines.register`). Those are the only sites that resolve
  one, all through the same `undeclaredIconProblem` rule. App icon fields take
  a BB icon name instead; paint plugin artwork there with
  `app.experimental_icons.register()` (see
  `references/frontend-components.md`), which shares this spelling but not this
