...
Use `script` when the output is fully determined by code: watchdogs, threshold alerts, health checks, heartbeats, and API pollers with a fixed output shape. Scripts run on the bb server. New scripts default to the project source on that host; existing scripts without a saved policy retain the legacy plugin-storage directory. Use `--working-directory legacy|project|<absolute-server-path>` to select the policy. Script automations do not have an environment field and do not accept environment flags.
