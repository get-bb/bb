...
export function requireRelativeMarkdownFile(value: unknown): string {
  const file = requireRelativeSourceFile(value);
  if (!MARKDOWN_EXTENSIONS.includes(path.posix.extname(file).toLowerCase())) {
    throw new Error(
      `"file" must end with .md or .markdown, got ${JSON.stringify(file)}`,
    );
  }
  return file;
}
