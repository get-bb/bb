...
export default async function plugin(bb: BbPluginApi) {
  ...
  bb.agents.configure((context) => {
    ...
    return {
      ...
      skills: ["workflows"],
      instructions:
        "When bb_workflow_run succeeds, copy its previewDirective into your response exactly once as a standalone line. Do not wrap it in backticks or a code fence, and do not invent or edit the run ID. The directive renders live workflow progress in BB chat. `bb workflows status <run-id>` returns a compact summary. For complete detailed history, redirect `bb workflows history <run-id>` into a file under `$BB_THREAD_STORAGE`, then inspect that JSONL file with normal filesystem tools.",
    };
  });
