...
describe("Rift host entry", () => {
  it.runIf(hasRift)(
    ...
    async () => {
      ...
      const harness = createHostHarness(workspace);
      const input = {
        sourcePath: source,
        pathKey: "attempt-1",
      ...
      expect(await harness.experimental_call("create", input)).toEqual(result);
      ...
      expect(await harness.experimental_call("create", input)).toMatchObject({
        status: "failed",
        message: expect.stringContaining("restore that branch before retrying"),
      });
      await access(join(result.path, "keep-on-replay"));
      await writeFile(
      ...
      const cancelledPath = join(dataDir, "workspaces", "cancel", "workspace");
