 ...
 * code, not the tests. Plugin code may import only `@get-bb/plugin-sdk` (and
 * its published subpaths), `zod`, node built-ins, and its own files; tests
 * may add `@get-bb/plugin-sdk/provider-bridge/testing` and the test runner.
 *
 * A `@bb/*` import would still typecheck and run inside this monorepo, which
 * is exactly why it needs a test: the workspace hides the privilege.
 */
...
/** Specifiers plugin code (server, host, bridge) may import. */
...
describe("echo-provider imports only the public SDK", () => {
  ...
  it("scans the plugin's source files", () => {
    ...
    expect(names).toContain(join("src", "provider-bridge.ts"));
  });
