...
| Script runtimes                | Run harmless bash, sh, node, and python3 fixtures producing known stdout/stderr and exit codes. Exercise legacy, project, and absolute-path working-directory policies, including a remote-only or missing project source. | Existing fieldless scripts decode as legacy; new scripts default to project; replacements preserve policy; each explicit policy uses only its selected server-host directory; unavailable selections fail clearly; failures summarize the first stderr line. Successful silent scripts do not fabricate assistant messages. |
