...
| Remove host | Attach visible and hidden threads, including running or stopping work, to disposable and persistent fixture machines. Remove through the UI or `machine remove`, then try reconnecting with old enrollment. | Removal requires no prior archiving. Work settles, provider-owned environments are cleaned up, and threads remain unarchived as read-only history. The sidebar and thread view show Machine removed with the old machine name retained in workspace details; composer, terminal, and workspace execution controls are unavailable. Pending removal and cleanup failure have distinct labels, and machine settings offers Retry cleanup. Manually enrolled machines use the same history UI even with retained environment records. Revoked hosts cannot reconnect; unrelated hosts and projects are unaffected. |
