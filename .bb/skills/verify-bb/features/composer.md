...
| Provider, model, reasoning, service tier | Open Provider, model and reasoning; select supported combinations and cycle forward/backward through keyboard actions. Turn off Settings → General → Allow fast service tier, then reopen the picker and inspect Fast mode. | Options reflect the live provider catalog, unsupported combinations are unavailable, and the created session uses the selection. Fast mode disappears while disallowed and returns when allowed. |
