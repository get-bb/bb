...
| Issue creation and editing | In the disposable repo, create an issue, edit body/title and offered metadata, then verify through gh. | Forge data matches submitted changes; canceled edits do not write externally. |
| Comments, labels, assignment, state | Add/edit a test comment and change supported labels/assignee/open state. | Only the chosen issue/PR receives changes; permissions/errors remain visible. |
...
| Agent interface | Compare CLI repos/issues/prs/sync with the panel and registered plugin commands. | Both surfaces use the same tracked cache and handle unknown repositories explicitly. |
