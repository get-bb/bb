...
function journal(): JournalLine[] {
  return [
    ...
    // A call with no result: must not count as a success.
...
test("parseJournal rejects an unknown event type", () => {
  assert.throws(
    () => parseJournal('{"seq": 0, "event": {"type": "teleport"}}'),
...
test("a journal without a terminal done is aborted and can be closed", () => {
  ...
  // Closing an already-complete journal is a no-op.
...
test("journalToTrajectory folds events into metrics", () => {
  ...
  // t1 errored, t4 never resolved -> two successes.
...
test("toGeneratorOutput refuses to fabricate tokens for a non-token run", () => {
  ...
  assert.throws(
    () => toGeneratorOutput([trainableRecord("ok", 1), eventOnly]),
    (error: unknown) => {
...
test("optional per-trajectory fields are all-or-nothing", () => {
  ...
  delete (withoutTime as { e2eTimeSec?: number }).e2eTimeSec;
  delete (withoutTime as { timeSplits?: unknown }).timeSplits;
  delete (withoutTime as { tokens?: unknown }).tokens;
  ...
  // One record has logprobs, the other does not -> null, matching the recipe.
...
test("compaction is counted, and a compacted run stays trainable", () => {
  // Middle truncation is legitimate recovery, not failure: the run must still be
  // usable for training. What must never happen is it looking identical to a run
...
test("rollout metrics expose the compaction rate of a batch", () => {
  // A reward shift with a simultaneous compaction shift has two explanations;
  // without these metrics a training run cannot tell which one it is looking at.
