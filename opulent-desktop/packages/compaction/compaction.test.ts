...
/** Filler with enough bulk that the estimator has something to free. */
...
test("a tool call is never separated from its result", () => {
  // The pair sits in the droppable middle, surrounded by bulk on both sides.
  ...
  assert.equal(
    hasCall,
    hasResult,
    "a toolCall and its toolResult must be dropped or kept together",
  );
});
...
test("an inputRequested is never separated from its resolution", () => {
  ...
  const asked = result.events.some(
    (e) => e.type === "inputRequested" && e.requestId === "q1",
  );
  const answered = result.events.some(
    (e) => e.type === "inputResolved" && e.requestId === "q1",
  );
...
test("a pair straddling the protected tail is left whole", () => {
  // toolResult lands inside keepTail, so the toolCall must not be dropped alone.
  const events: AgentEvent[] = [
    started,
    ...filler(40),
...
test("a transcript that cannot be compacted reports failure instead of looping", () => {
  // Everything is protected by keepHead/keepTail, so nothing is droppable.
...
test("compaction rejects an incoherent policy rather than guessing", () => {
  assert.throws(
    () => compactMiddle([started], { keepHead: -1, keepTail: 0, targetTokens: 10 }),
    /non-negative/,
  );
  assert.throws(
    () => compactMiddle([started], { keepHead: 0, keepTail: 0, targetTokens: 0 }),
    /must be positive/,
  );
});
...
test("the compacted window renumbers contiguously and survives the journal codec", () => {
  ...
  // The window is itself a valid journal — the resume path writes it back out.
