...
test("opencode and Devin CLI launch as ACP subprocesses", () => {
  // bb ships `opencode acp` today; `devin acp` is documented as a stdio JSON-RPC
  // subcommand for ACP clients, so both register through the same schema.
...
test("every local-acp provider carries skill roots including the Opulent convention", () => {
  for (const provider of DEFAULT_PROVIDERS) {
    if (provider.providerClass !== "local-acp") continue;
    ...
    assert.ok(
      all.includes(".agents/skills"),
      `${provider.id} does not resolve .agents/skills`,
    );
  }
});
...
test("Claude Managed Agents splits the control plane from the sandbox worker", () => {
  ...
  // The hosted loop needs somewhere to execute; local-only is not a valid pairing.
...
test("no provider claims token fidelity yet", () => {
  // Tier-2 RL data requires Opulent to own the sampler. No vendor-hosted agent
  // qualifies, and asserting it here keeps a future edit honest.
  for (const provider of DEFAULT_PROVIDERS) {
    assert.notEqual(
      provider.maxFidelity,
      "token",
      `${provider.id} claims token fidelity; only an Opulent-served model may`,
    );
  }
});
...
test("every provider is reachable on at least one execution target", () => {
  for (const provider of DEFAULT_PROVIDERS) {
    assert.ok(
      provider.executionTargets.length > 0,
      `${provider.id} has no execution targets`,
    );
  }
});
