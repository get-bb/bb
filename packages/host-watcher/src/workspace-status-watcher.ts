...
// Glob entries are matched against the root-relative path of every directory
// parcel crawls, and a match skips the whole subtree. The Git-derived ignore
// list below only covers the root's own top-level ignored directories; an
// "umbrella" root with untracked nested checkouts, or a root that is not a
// repository, otherwise gets one inotify watch per nested directory and can
// OOM the host (get-bb/bb#1779). `*/**/.git` skips nested repositories but
// keeps `<root>/.git` watchable so a plain directory can still be promoted
// after `git init`.
