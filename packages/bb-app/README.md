...
`reasoningCli` or `nativeReasoning` reasoning settings, and `nativeSkillRoots`
for provider-native skill directories. A `logo` field accepts an SVG, PNG, or
WebP path for the provider picker icon.
