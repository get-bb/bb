...
import { ACP_PROTOCOL_VERSION, AcpTranslator, decodeToolCall, isTruncatedStop, stopReasonToStatus, type SessionNotification, type StopReason, } from "./acp.ts";
...
test("agent message chunks become text deltas and thoughts become reasoning deltas", () => {
    ...
    assert.deepEqual(t.translate(sess({
...
test("non-text content blocks and empty text produce no timeline event", () => {
    const t = new AcpTranslator();
    assert.deepEqual(t.translate(sess({ sessionUpdate: "agent_message_chunk", content: { type: "image", data: "..." } })), []);
    assert.deepEqual(t.translate(sess({ sessionUpdate: "agent_message_chunk", content: { type: "text", text: "" } })), []);
});
...
test("a tool call settles exactly once, and failure is preserved", () => {
    const t = new AcpTranslator();
    t.translate(sess({ sessionUpdate: "tool_call", toolCallId: "c1", title: "Read", kind: "read", status: "pending" }));
...
test("usage_update records context occupancy without inventing an output split", () => {
    const t = new AcpTranslator();
    assert.deepEqual(t.translate(sess({ sessionUpdate: "usage_update", used: 53000, size: 200000 })), [{ type: "usage", inputTokens: 53000, outputTokens: 0 }]);
});
test("UI-only and unknown future updates are dropped, not thrown on", () => {
    ...
    assert.deepEqual(t.translate(sess({ sessionUpdate: "user_message_chunk", content: { type: "text", text: "hi" } })), []);
});
