...
import { admitTask, feedbackFor, probeReward, screenBatch, type ProbeResult, type TaskProposal, } from "./envauthor.ts";
...
test("a task whose F2P tests already pass is rejected as free reward", () => {
    const admission = admitTask(proposal({ baseProbe: { failToPass: { test_empty_input: true }, passToPass: { test_basic: true } } }));
    assert.equal(admission.admitted, false);
    assert.ok(admission.rejections.some((r) => r.code === "trivially_passing"));
});
test("a task the oracle cannot solve is rejected", () => {
    const admission = admitTask(proposal({ oracleProbe: { failToPass: { test_empty_input: false }, passToPass: { test_basic: true } } }));
    assert.equal(admission.admitted, false);
    assert.ok(admission.rejections.some((r) => r.code === "oracle_fails"));
});
test("an oracle that breaks the regression guard is rejected", () => {
    const admission = admitTask(proposal({ oracleProbe: { failToPass: { test_empty_input: true }, passToPass: { test_basic: false } } }));
    assert.equal(admission.admitted, false);
...
test("identical tasks are deduplicated by content hash", () => {
    const a = proposal();
    const b = proposal({ authoredBy: "sub-agent-2" });
