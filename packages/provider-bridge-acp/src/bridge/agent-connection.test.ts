...
describe("ACP agent stdio lifecycle", () => {
  ...
  // bb-fork(windows): Windows stdio/exit semantics; closing stdin does not reap
  // the child the way POSIX does.
  ...
  // bb-fork(windows): a killed Windows child reports SIGTERM, not exit code 0.
  it.skipIf(process.platform === "win32")(
    "makes an intentionally stopped connection unavailable before stdin teardown",
    async () => {
      const ready = deferred<void>();
      const exited = deferred<AcpAgentExitInfo>();
      const connection = createAcpAgentConnection({
