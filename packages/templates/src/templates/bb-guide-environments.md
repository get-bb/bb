...
Making your repo work with bb:
  ...
  checkout, and worktree rows start the same command from that worktree. The run
  appears as a pinned Run tab in the thread's terminal pane (the bottom dock on
  desktop, the drawer's terminal strip on mobile). While the terminal is starting
  or running, the button becomes a stop button and the sidebar shows the active
  run state.
