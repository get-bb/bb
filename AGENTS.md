...
- Use the shared Silk-backed responsive drawer (`ResponsiveDrawerShell` / `@silk-hq/components`) for every compact slide-out menu, picker, popover, dialog, and the compact sidebar. Desktop overlays stay Radix-backed. Do not use modal drawer primitives that add `inert` or `aria-hidden` to the app root: iOS Safari can recalculate styles for the full app tree and stall the interaction. Configure Silk with `inertOutside={false}`, start presentation before heavy content, realize that content after two animation frames with a timeout fallback, and retain it after the first open. Import `@silk-hq/components/layered-styles.css` before Tailwind. Verify representative drawers in iOS Simulator Safari and protect the app-root and deferred-realization behavior with tests.
