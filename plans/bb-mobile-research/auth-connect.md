...
Public API + `/ws` are unauthenticated; the server is loopback-only and remote access must use the authenticated bb connect/Tailscale boundary (`docs/configuration.md:849-855`, `docs/multiple-devices.md:38-41`). Direct LAN binding is rejected; a phone should use `http(s)` through Tailscale Serve or bb connect; CORS (`server.ts:307-319`, allowlist `packages/config/src/local-app-origins.ts:38-62`) and `browserRequestProblem` only bite when an Origin header is sent (RN iOS WebSocket may send `Origin` = ws URL origin — equal to request origin → allowed by `browser-request-guard.ts:127-129`). Tailscale Serve: `tailscale serve --https=443 http://127.0.0.1:38886` + `BB_APP_URL` (`docs/multiple-devices.md:19-29`); the browser guard accepts only configured app origins and does not expand trust from `Host`/`X-Forwarded-Host` (`browser-request-guard.ts:69-133`); generated URLs still use configured app or proxy metadata (`apps/server/src/routes/system.ts:84-109`). Plugin RPC routes (`/api/v1/plugins/:id/rpc/:method`) enforce "local" auth = Origin check + JSON content-type for mutations (`apps/server/src/routes/plugins.ts:126-133,608-616`) — a native client with no Origin and JSON body passes, so a phone on LAN/Tailscale can call `createMachineCode` and self-enroll a machine credential for later roaming via connect.
...
- The server API is unauthenticated but loopback-only (start-server.ts:39-53); direct LAN binding and `http://<ip>:38886` access are rejected. Remote phones must use the authenticated bb connect route or a private Tailscale Serve boundary.
