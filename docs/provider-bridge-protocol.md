...
Requests routed through a required machine integration carry
`executionIntegration: { id, providerId }`. `providerId` is the selected harness;
`options.model` remains its selected model. Ordinary provider requests omit
this field. Integrations must implement their declared capabilities and model
catalogs. Release detaches the BB client; it must not cancel external execution
or recording. Server-side session ownership prevents resuming the same session
through a different machine integration.
