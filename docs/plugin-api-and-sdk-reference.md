...
| `bb.branding.icon`       | conditional | non-empty compact identity. Ordinary strings remain host icon-name hints; a value beginning with `./` is a plugin-relative monochrome SVG asset. Compact surfaces prefer the asset, then the named manifest icon, a contribution icon, and the generic plugin icon. |
...
`bb` is strict: unknown fields fail manifest validation. Manifest paths must be non-empty and relative, may not escape the plugin directory lexically or through a branding symlink, and declared server/branding/theme files must exist. Plugin-owned compact icons must be valid SVG files inside the plugin root. The host snapshots and hashes their exact bytes, serves them at an immutable URL, and renders them as `currentColor` CSS masks without rereading the mutable source file. Rich `branding.logo` behavior is unchanged. Theme CSS must end in `.css`; duplicate theme ids fail. The server entry is required and must exist; the optional app entry is resolved under the same root. Builtin-reserved ids cannot be installed from non-builtin sources. Manifest errors are human-readable and put the plugin in an install/load error rather than accepting ignored fields.

Plugin inventory exposes a resolved compact asset as nullable `experimental_iconUrl`; consumers normalize older responses that omit it to `null`. Before stabilizing this field, audit real third-party icon legibility across compact sizes and themes, whether monochrome masks cover valid use cases, and whether the final inventory should retain a URL field or use a discriminated icon source.
...
- **Experimental compact icons:** `bb.branding.icon` keeps ordinary host icon names backward-compatible while `./...` opts into a validated plugin-owned SVG. The inventory URL remains `experimental_iconUrl` until the compact-size, theme, monochrome, and final source-shape questions above are closed.
