...
`PluginEnvironmentProviderDefinition.presentation` (SDK 0.4.62) optionally declares `{ groupsThreads: boolean }`. Sidebar grouping defaults to the real `isWorktree` fact when absent. This does not change Git semantics or ownership. Audit third-party copy providers and fallback behavior before stabilization.
