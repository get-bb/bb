...
`Control+Shift+[/]` on the web. They move back and forward through the threads
you opened in the current window, like browser history, rather than through the
sidebar order. Opening a different thread after going back discards the forward
entries.
