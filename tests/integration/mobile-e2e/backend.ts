...
//
// The backend always binds to loopback. The iOS Simulator shares the Mac
// loopback, so the app can use http://127.0.0.1:<port> directly. The Android
// emulator uses 10.0.2.2. Physical-device testing should use bb connect or a
// private Tailscale Serve route instead of exposing this unauthenticated server.
...
async function main(): Promise<void> {
  ...
  const harness = await createIntegrationHarness({ serverPort });
