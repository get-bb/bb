...
describe("RuntimeManager", () => {
  ...
  it("refreshes cached provider environments after machine overrides change or are removed, deferring while work is active", async () => {
    let machineEnv: Record<string, string> = { MACHINE_ENV_VERIFY: "initial" };
    ...
    const manager = new RuntimeManager({
      provisionWorkspace: createProvisionWorkspaceMock("/tmp/env-1"),
      createRuntime,
      applyMachineEnvironment: (shell) => ({ ...shell, ...machineEnv }),
    });
    const args = { environmentId: "env-1", workspacePath: "/tmp/env-1" };
    try {
      const initial = await manager.ensureEnvironment(args);
      const runtime = createRuntime.mock.results[0]?.value;
      if (!runtime) throw new Error("Missing runtime");
      runtime.setOpenBackgroundWork(true);
      machineEnv = { MACHINE_ENV_VERIFY: "updated" };
      expect((await manager.ensureEnvironment(args)).runtime).toBe(
        initial.runtime,
      );
      expect(runtime.shutdown).not.toHaveBeenCalled();
      runtime.setOpenBackgroundWork(false);
      const updated = await manager.ensureEnvironment(args);
      expect(updated.runtime).not.toBe(initial.runtime);
      expect(createRuntime.mock.calls[1]?.[0].shellEnv).toEqual(machineEnv);
      machineEnv = {};
      expect((await manager.ensureEnvironment(args)).runtime).not.toBe(
        updated.runtime,
      );
      expect(createRuntime.mock.calls[2]?.[0].shellEnv).not.toHaveProperty(
        "MACHINE_ENV_VERIFY",
      );
      expect((await manager.ensureEnvironment(args)).runtime).toBe(
        manager.get("env-1")?.runtime,
      );
  ...
  it("refreshes the provider maintenance environment after machine overrides are removed", async () => {
    const dataDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "bb-machine-maintenance-"),
    );
    let machineEnv: Record<string, string> = { MACHINE_ENV_VERIFY: "initial" };
    const createRuntime = vi.fn((_options: AgentRuntimeOptions) =>
      createFakeRuntime(),
    ...
    try {
      const first = await manager.ensureProviderMaintenanceRuntime({ dataDir });
      machineEnv = {};
      const second = await manager.ensureProviderMaintenanceRuntime({
        dataDir,
      });
      expect(second).not.toBe(first);
      expect(first.shutdown).toHaveBeenCalledOnce();
      expect(createRuntime.mock.calls[1]?.[0].shellEnv).not.toHaveProperty(
        "MACHINE_ENV_VERIFY",
      );
    } finally {
      await manager.shutdownAll();
