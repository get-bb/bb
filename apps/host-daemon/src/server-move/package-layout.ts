...
export interface BbAppPackageEntries {
  packageRoot: string;
  bbAppEntry: string;
...
export async function resolveFullBbAppPackage(
  packageRoot: string,
): Promise<BbAppPackageEntries | null> {
  const entries = {
    packageRoot,
    bbAppEntry: join(packageRoot, "dist", "bb-app.js"),
    bbServerEntry: join(packageRoot, "dist", "bb-server.js"),
  };
  const required = [
    entries.bbAppEntry,
    entries.bbServerEntry,
