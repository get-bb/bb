...
/** User-authored messages in list order, for the table-of-contents sheet. */
export interface TimelineTableOfContentsEntry {
  key: string;
  rowId: string;
  preview: string;
  createdAt: number;
...
const TOC_PREVIEW_MAX_CHARS = 120;

export function buildTimelineTableOfContents(
  items: readonly TimelineListItem[],
): TimelineTableOfContentsEntry[] {
  const entries: TimelineTableOfContentsEntry[] = [];
  for (const item of items) {
    if (item.kind !== "conversation:user" || item.depth !== 0) continue;
    const preview = item.row.text.replace(/\s+/g, " ").trim();
    entries.push({
      key: item.key,
      rowId: item.row.id,
      preview:
        preview.length > TOC_PREVIEW_MAX_CHARS
          ? `${preview.slice(0, TOC_PREVIEW_MAX_CHARS - 1)}…`
          : preview.length > 0
            ? preview
            : attachmentOnlyPreview(item.row),
...
function attachmentOnlyPreview(row: TimelineUserConversationRow): string {
  const attachments = row.attachments;
  if (!attachments) return "(empty message)";
  const images = attachments.webImages + attachments.localImages;
  const parts: string[] = [];
  if (images > 0) parts.push(`${images} image${images === 1 ? "" : "s"}`);
  if (attachments.localFiles > 0) {
    parts.push(
      `${attachments.localFiles} file${attachments.localFiles === 1 ? "" : "s"}`,
    );
  }
