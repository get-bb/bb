...
function renderPluginCommandsSkill(
  ...
  return [
    ...
    `Buffered stdout and stderr is capped at ${PLUGIN_CLI_OUTPUT_MAX_BYTES} UTF-8 bytes. Above-limit`,
    "buffered results fail atomically as `plugin_cli_output_too_large` and are never clipped;",
    "commands that advertise streaming can emit larger stdout without user action.",
    "",
    ...sections,
    "",
  ].join("\n");
