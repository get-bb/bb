...
import {
  ...
  type PluginRpcError,
  type ExperimentalPluginRpcHandlerContext,
...
export function createPluginService(deps: PluginServiceDeps): PluginService {
  ...
  return {
    ...
    async invokeRpcHandler(id, method, handler, input, signal) {
      const outcome = await invokeWrapped(id, `rpc ${method}`, async () => {
        ...
        const context: ExperimentalPluginRpcHandlerContext = { experimental_signal: signal };
