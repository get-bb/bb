...
export interface PluginService {
  ...
  searchMentions(args: {
    ...
    provider?: { pluginId: string; providerId: string };
  ...
  return {
    ...
    async searchMentions(args) {
      ...
      for (const [id, plugin] of entries) {
        if (args.provider && args.provider.pluginId !== id) continue;
        for (const record of [...plugin.handle.mentionProviders]) {
          if (args.provider && args.provider.providerId !== record.id) continue;
