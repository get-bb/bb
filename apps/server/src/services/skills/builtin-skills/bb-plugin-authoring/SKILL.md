...
const settings = bb.settings.define({
  ...
  preamble: { type: "string", label: "Preamble", multiline: true, default: "" }, // textarea, not a one-line input
...
`multiline: true` renders a string in a textarea on its own full-width row,
for prose and other newline-bearing text. The stored value is still a plain
string, so nothing about `get()` changes. It composes with `secret` — use
both for a PEM key or service-account JSON. A secret textarea stays
write-only (empty, with a `[set]`/`[not set]` placeholder) but is not
masked: a textarea has no password mode, and hiding a pasted key would
obscure the paste without protecting a value the server never sends back.
