...
- `useComposer()` → programmatic access to the chat composer draft (the
  ...
  `threads.send`/`threads.spawn` would silently schedule a different message.
  There is no "submit now" arm. It rejects with a message you can show when
  the composer refuses — a queued-message editor and a side chat have no
  schedulable submit — and rejects after the host restores the draft when the
  request itself fails. Reference: `plugins/scheduled-send/app.tsx`;
  `focus()` focuses the caret; `scope` reports
...
>>>>>>> ba9101fba0 (Scheduled sends via composer submit pipeline and host dialog)
