...
- Pass `--plugin-input <pluginId>=<json>` (repeatable) to `bb thread spawn` or
  `bb thread tell` to address one plugin's dispatch gates. The value is JSON, so
  quote a string as `'"text"'`; each plugin sees only its own entry, and a later
  flag for the same plugin replaces the earlier value. The input rides the hold
...
>>>>>>> a1e5e358a4 (Queue rework wave 1: foundations)
