...
- For failed threads, inspect `bb thread show <id> --json` and
  ...
  user.
- Use `bb thread retry <id>` to ask for a new retry of the thread's failed turn.
  It re-submits that turn by reference — the same prompt blocks, no duplicated
  user message in the timeline — and increments the attempt number (2 is the
  first retry). With no `--turn` it retries the most recent turn, the one whose
  failure put the thread in `error`; `--turn <requestId>` asserts which turn you
  mean and fails when the thread has moved on. It errors when the thread has not
  failed or `--turn` names a different turn (409 `no_failed_turn`), and when that
  turn already has a retry queued (`retry_already_queued`). Add
  ...
  still queue behind a busy thread or a plugin's dispatch hook.
  `--reason <text>` labels the queued row. The SDK equivalent is
...
- Use `bb provider-retry retry <thread-id>` to release a retry the plugin has
  already scheduled, sending it now instead of waiting for its reset. It does
  not create one: with nothing scheduled, ask for a new retry with
  `bb thread retry <thread-id>`.
...
>>>>>>> e5f55adb06 (One narration surface: cut queue-state timeline rows; queue-driven sidebar)
