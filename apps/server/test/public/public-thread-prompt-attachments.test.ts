...
describe("public thread prompt attachments", () => {
  ...
  it("normalizes an absolute image before a send enters the dispatch queue", async () => {
    await withTestHarness(async (harness) => {
      const { host, session } = seedHostSession(harness.deps, {
        id: "host-dispatch-queue-image",
      });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
        path: "/remote/project",
      ...
      seedThreadRuntimeState(harness.deps, {
        environmentId: environment.id,
        providerThreadId: "provider-dispatch-queue-image",
        threadId: thread.id,
      });
      seedTurnStarted(harness.deps, {
        ...
        sequence: 3,
        threadId: thread.id,
        turnId: "turn-dispatch-queue-image",
      });
      const absoluteImagePath = "/remote/references/dispatch-queued.png";
      registerRemoteImageResponder(harness, {
        hostId: host.id,
        paths: [absoluteImagePath],
        sessionId: session.id,
      ...
      expect(response.status).toBe(200);
      const result = sendMessageResponseSchema.parse(await readJson(response));
      expect(result.delivery).toBe("queued");
      if (result.delivery !== "queued") {
        throw new Error("Expected queued delivery");
      }
      expect(result.queuedMessage.content).toEqual([
        {
          type: "localImage",
          path: expect.stringMatching(
            /^dispatch-queued-\d+-[a-z0-9]{6}\.png$/u,
          ),
        },
      ]);
    });
  ...
  it("normalizes an absolute image in a fork prompt on the source execution host", async () => {
    await withTestHarness(async (harness) => {
      ...
      const fork = threadSchema.parse(responseBody);
