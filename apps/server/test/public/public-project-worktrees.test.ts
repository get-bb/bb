...
describe("GET /projects/:id/worktrees", () => {
  it("merges discovery with environments by canonical identity and excludes the source", async () => {
    await withTestHarness(async (harness) => {
      ...
      const envManagedReady = seedEnvironment(harness.deps, {
        hostId: host.id,
        projectId: project.id,
        path: "/tmp/wtproj/bb-ready",
        status: "ready",
        providerOwnsPath: true,
        environmentProviderId: "git-worktree",
        branchName: "bb/ready",
        ...
        handle: () =>
          respondWith({
            worktrees: [
              ...
              worktreeEntry({
                path: "/private/tmp/wtproj/bb-ready",
                branch: "bb/ready",
              }),
              worktreeEntry({
            ...
            resolvedPaths: [
              ...
              {
                path: "/tmp/wtproj/bb-ready",
                canonicalPath: "/private/tmp/wtproj/bb-ready",
              },
              {
                path: "/tmp/other/off-registry",
      ...
      expect(byPath.get("/private/tmp/wtproj/bb-ready")).toMatchObject({
        environmentId: envManagedReady.id,
        environmentProviderId: "git-worktree",
  ...
  it("does not scan ephemeral machines", async () => {
    await withTestHarness(async (harness) => {
      const { host, session } = seedHostSession(harness.deps, {
        id: "host-wt-persistent",
      });
      const sandbox = upsertHost(harness.db, harness.deps.hub, {
        id: "host-wt-sandbox",
        name: "Sandbox",
        type: "ephemeral",
      });
      ...
      expect(response.status).toBe(200);
      const body = (await readJson(response)) as {
        worktrees: Array<{ hostId: string }>;
        failures: unknown[];
      };
      expect(body.worktrees.map((worktree) => worktree.hostId)).toEqual([
        host.id,
      ]);
      expect(body.failures).toEqual([]);
    });
