...
import { createAgentRuntime, type AgentRuntime } from "@bb/agent-runtime";
...
describe("echo-provider canary: plugin install → server command → runtime → ingest", () => {
  ...
  beforeEach(async () => {
    ...
    // The declaration names this variable in `experimental_env.passthrough`;
  ...
  it("persists every grammar v3 capability the third-party bridge emits", async () => {
    ...
    // The extension item, validated against the declared schema.
