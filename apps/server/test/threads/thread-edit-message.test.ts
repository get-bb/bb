...
const EDIT_MESSAGE_TEST_ACTOR: ActorStamp = {
  principalId: "human:edit-message-test",
  principalKind: "human",
  displayName: "Edit Message Tester",
};
...
describe("editThreadMessage", () => {
  it("preserves an agent caller and applies agent permission policy", async () => {
    await withTestHarness(async (harness) => {
      ...
      await expect(
        editThreadMessage(harness.deps, { actor: EDIT_MESSAGE_TEST_ACTOR,
          environment,
          thread,
          payload: {
  ...
  it("targets the latest user message when expectedRequestSequence is omitted, skipping agent turns", async () => {
    await withTestHarness(async (harness) => {
      ...
      const editPromise = editThreadMessage(harness.deps, { actor: EDIT_MESSAGE_TEST_ACTOR,
        environment,
        thread,
        payload: {
  ...
  it("returns the committed result to an overlapping retry of the same operation", async () => {
    await withTestHarness(async (harness) => {
      ...
      const firstEdit = editThreadMessage(harness.deps, { actor: EDIT_MESSAGE_TEST_ACTOR,
        environment,
        thread,
        payload,
      });
      const secondEdit = editThreadMessage(harness.deps, { actor: EDIT_MESSAGE_TEST_ACTOR,
        environment,
        thread,
        payload,
      });
