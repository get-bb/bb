...
describe("CLI command output contracts", () => {
  ...
  it("bb status-state list prints values", async () => {
    const list = vi.fn(async () => ({
      values: { tasks: ["one"] },
      versions: { tasks: "hash-1" },
      hash: "list-hash",
    }));
    createClientMock.mockReturnValue(
      asServerClient({
    ...
    await runCommand(["status-state", "list", "thread-1"], (program) =>
      registerStatusStateCommand(program, () => "http://server"),
    );
  ...
  it("bb status-state get prints one raw JSON value", async () => {
    const get = vi.fn(async () => ({
      key: "tasks",
      value: ["one"],
      version: "hash-1",
    ...
    await runCommand(["status-state", "get", "thread-1", "tasks"], (program) =>
      registerStatusStateCommand(program, () => "http://server"),
    );

    expect(get).toHaveBeenCalledWith({
      param: { id: "thread-1", key: "tasks" },
    });
    expect(collectLogPayloads(vi.mocked(console.log)).join("\n")).toContain(
      '"one"',
    );
  ...
  it("bb status-state set parses JSON and forwards CAS headers", async () => {
  ...
  it("bb status-state delete forwards must-exist precondition", async () => {
    const del = vi.fn(async () => ({ ok: true }));
    createClientMock.mockReturnValue(
      asServerClient({
        api: {
    ...
    await runCommand(
      ["status-state", "delete", "thread-1", "tasks", "--must-exist"],
      (program) => registerStatusStateCommand(program, () => "http://server"),
    );
    ...
    expect(collectLogLines(vi.mocked(console.log))).toContain("Deleted tasks");
  });

  it("bb status-state set rejects invalid JSON values before writing", async () => {
    const put = vi.fn(async () => ({
      key: "tasks",
      value: [],
      version: "hash-1",
    ...
    await expect(
      runCommand(
        ["status-state", "set", "thread-1", "tasks", "not-json"],
        (program) => registerStatusStateCommand(program, () => "http://server"),
      ),
    ).rejects.toThrow("process.exit:1");
  ...
  it("bb status-state set reports @file read failures before writing", async () => {
    const put = vi.fn(async () => ({
      key: "tasks",
      value: [],
      version: "hash-1",
    ...
    await expect(
      runCommand(
        [
          ...
          "thread-1",
          "tasks",
          "@/tmp/bb-status-state-missing-file.json",
        ],
        (program) => registerStatusStateCommand(program, () => "http://server"),
      ),
    ).rejects.toThrow("process.exit:1");
    ...
    expect(collectLogLines(vi.mocked(console.error)).join("\n")).toContain(
      "bb-status-state-missing-file.json",
    );
  });

  it("bb status-state set rejects mutually exclusive CAS flags before writing", async () => {
    const put = vi.fn(async () => ({
      key: "tasks",
      value: [],
      version: "hash-1",
    ...
    expect(put).not.toHaveBeenCalled();
    expect(collectLogLines(vi.mocked(console.error))).toContain(
      "Error: Use only one of --if-match, --create-only, or --must-exist.",
    );
  });

  it("bb status-state help lists subcommands and CAS flags", async () => {
    const output = await getHelpOutput(["status-state"], (program) =>
      registerStatusStateCommand(program, () => "http://server"),
    );

    expect(output).toMatchInlineSnapshot(`
      "Usage: bb status-state [options] [command]

      Read and write persistent reactive STATUS state

      Options:
        -h, --help                                    display help for command

      Commands:
        list [options] <threadId>                     List all STATUS state keys and values
        get [options] <threadId> <key>                Read one STATUS state JSON value
        set [options] <threadId> <key> <valueOrFile>  Write one STATUS state JSON value
        delete [options] <threadId> <key>             Delete one STATUS state JSON value
        help [command]                                display help for command
      "
    `);

    const setOutput = await getHelpOutput(["status-state", "set"], (program) =>
      registerStatusStateCommand(program, () => "http://server"),
    );
    expect(setOutput).toContain("--if-match <version>");
    expect(setOutput).toContain("--create-only");
    expect(setOutput).toContain("--must-exist");
  });
