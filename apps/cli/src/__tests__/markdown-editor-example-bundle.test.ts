...
// The Markdown Editor example bundles Milkdown Crepe (prosemirror +
// codemirror) — a large graph that blows the 5s default on cold CI runners.
vi.setConfig({ testTimeout: 120_000 });
import { buildPluginApp } from "@bb/plugin-build";

/**
 * Evaluates the Markdown Editor example's built bundle against a stub runtime
 * (the github-example-bundle.test.ts pattern) and asserts its default export
 * registers the nav panel, thread panel action, and markdown file opener.
 */
const MARKDOWN_EDITOR_DIR = fileURLToPath(
  new URL("../../../../examples/plugins/markdown-editor", import.meta.url),
);
...
describe("markdown editor example frontend bundle", () => {
  let root: string;

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "bb-markdown-editor-bundle-"));
  });
  ...
  it("registers the Markdown Editor plugin surfaces", async () => {
    const pluginDir = join(root, "markdown-editor");
    await cp(MARKDOWN_EDITOR_DIR, pluginDir, {
      recursive: true,
      filter: (source) => {
    ...
    await symlink(
      join(MARKDOWN_EDITOR_DIR, "node_modules"),
