...
/**
 * A thread of one of `pluginId`'s providers is open: load that provider
 * plugin's frontend now (its bundle was deferred at boot). A no-op for a
 * plugin already wanted; before boot the gate alone is enough, because
 * boot's reconcile reads it.
 */
export function requestProviderPluginFrontend(pluginId: string): void {
  if (markProviderPluginFrontendWanted(pluginId)) {
