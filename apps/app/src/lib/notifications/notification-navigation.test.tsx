...
async function setup(
  ...
  if (surface === "center") {
    act(() => openNotificationCenter());
    await act(async () => {
      await vi.advanceTimersByTimeAsync(500);
    });
  }
  return surface === "center"
    ? within(screen.getByTestId("notification-center"))
    : screen;
}
...
it.each([
  ...
  "dismisses only the %s notification used to open %s",
  async (surface, path) => {
    const notification = await setup(
      surface,
      `${path}?from=notification#latest`,
    );

    fireEvent.click(notification.getByRole("link", { name: "Open thread" }));
    await act(async () => {
      await vi.advanceTimersByTimeAsync(500);
    });
    ...
    expect(getNotifications().map((notification) => notification.toastId)).toEqual([
      "other",
    ]);
  },
);
...
it("keeps the notification when navigating to a non-thread route", async () => {
  const notification = await setup("center", "/settings");
  fireEvent.click(notification.getByRole("link", { name: "Open thread" }));
