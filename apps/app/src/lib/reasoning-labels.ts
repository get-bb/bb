...
/** The provider's declared description for a level, when it gave one. */
export function reasoningLevelDescription(
  level: ReasoningLevel,
  provider: ReasoningLabelSource | undefined,
): string | undefined {
  return provider?.reasoningLevels?.find((option) => option.id === level)
    ?.description;
}

/** The tier id bb's execution options send when fast mode is on. */
export const FAST_SERVICE_TIER_ID = "fast";
