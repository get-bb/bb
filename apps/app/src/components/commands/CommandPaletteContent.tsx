...
import type { CommandPaletteProps, PaletteOpenRequest } from "./CommandPalette";

export function CommandPaletteContent({
  threadId,
  projectId,
  request,
}: CommandPaletteProps & { request: PaletteOpenRequest }) {
  ...
  const handledRequest = useRef<PaletteOpenRequest | null>(null);
  useEffect(() => {
    if (handledRequest.current === request) return;
    handledRequest.current = request;
    prepareOpen(request.target);
    setSearchingThreads(request.mode === "threads");
    setOpen(true);
  }, [prepareOpen, request]);
