...
describe("MachinesSettingsSection", () => {
  ...
  it("points the explainer at Move server here", async () => {
    ...
    expect(
      await screen.findByText(
        /To use a different computer as the server, choose Move server here in its menu\.$/u,
      ),
    ).toBeDefined();
  });
  ...
  it("keeps Move server here and its explainer hidden while the serverMove experiment is off", async () => {
    ...
    expect(screen.getByText(/running threads may stop\.$/u)).toBeDefined();
    expect(screen.queryByText(/choose Move server here/u)).toBeNull();
