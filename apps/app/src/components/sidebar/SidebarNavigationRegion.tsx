...
/**
 * Owns the complete bounded navigation replacement. Search swaps in BB's
 * combobox above the retained thread-list mount; drawer, list, footer, resize,
 * and shortcut ownership remain in AppSidebar.
 */
export function SidebarNavigationRegion(props: BuiltInSidebarNavigationProps) {
    ...
    () =>
      createSidebarNavigationItems({
        ...
        newThreadShortcut: newThreadShortcut
          ...
          : null,
        searchThreadsDisabled: props.threadSearch === undefined,
  ...
  }, [items, navPanels, navigate, props, replacementIdentity, splitActions]);
    ...
    (
      ...
      activateSidebarNavigationItem(
        ...
        options.openInSplit,
        {
          ...
          searchThreads: () => current.props.threadSearch?.onActivate(),
          openExtensions: () => {
  ...
  // Activating Search replaces only the bounded nav controls with this
  // owner-rendered field. The thread list below remains mounted and receives
  // the live query through its existing host contract.
  if (props.threadSearch?.isActive) {
    return (
      <div
        data-testid="app-sidebar-primary-actions"
        className="shrink-0 px-2 py-2 group-data-[collapsible=icon]:hidden"
      >
        <ProjectListSearchInput threadSearch={props.threadSearch} />
      </div>
    );
  }
