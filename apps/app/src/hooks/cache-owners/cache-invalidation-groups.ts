 ...
 * `thread-created` / `thread-deleted`. Deliberately excludes both the full
 * conversation outline and turn-summary details. The realtime cache registry
 * refreshes the outline independently at stable conversation boundaries;
 * completed turn details are immutable. Mutations that can rewrite history
